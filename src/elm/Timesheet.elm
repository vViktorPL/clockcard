...
import Html.Attributes exposing (attribute, checked, class, classList, datetime, disabled, type_)
...
import Time.Extra exposing (durationHumanReadable, encodePosix, posixDecoder, viewPosix)
...
type Period
    = Period Posix Posix (List Integrations.LogRef)
...
update integrationConfigs msg model =
    case msg of
        SwitchTab tab ->
            ( switchTab model tab, Cmd.none )

        StartNewPeriodRequested ->
            ( model, Task.perform StartNewPeriod Time.now )

        StartNewPeriod startTime ->
            ( Maybe.withDefault model (startPeriod model startTime), Cmd.none )

        EndCurrentPeriodRequested ->
            ( model, Task.perform EndCurrentPeriod Time.now )

        EndCurrentPeriod endTime ->
            ( Maybe.withDefault model (addPeriod model endTime), Cmd.none )

        TogglePeriodSelection period ->
            ( toggleFinishedPeriodSelection model period, Cmd.none )

        RemoveSelectedPeriods ->
            ( removeSelectedPeriods model, Cmd.none )
...
stopwatch periodInProgress cumulatedTicks =
    case periodInProgress of
        Just startTime ->
            Stopwatch.running cumulatedTicks startTime

        Nothing ->
            Stopwatch.paused cumulatedTicks
...
view integrationsConfigs (Timesheet model) currentTime =
    let
        formOpened =
            JiraIssueLogManager.isFormOpened model.integrations.jira
    in
    ...
    Html.div [ classList [ ( "stopwatch", True ), ( "collapsed", collapsed ) ] ]
        ...
        , Html.div [ class "stopwatch__controls" ]
            [ case model.periodInProgress of
                Nothing ->
                    Html.button [ onClick StartNewPeriodRequested ] [ Html.text "Start ▶" ]

                Just _ ->
                    Html.button [ onClick EndCurrentPeriodRequested ] [ Html.text "❚❚ Pause" ]
            ]
        ]
...
startPeriod (Timesheet model) start =
    case model.periodInProgress of
        ...
        Nothing ->
            Just (Timesheet { model | periodInProgress = Just start })
...
getCurrentlyRunningPeriodStart (Timesheet model) =
    model.periodInProgress
...
viewPeriodsActions model =
    let
        actionsDisabled =
            countSelectedItems model.finishedPeriods == 0

        selectedDuration =
            countSelectedDuration model.finishedPeriods
    in
    Html.div []
...
periodToDuration (Period start end _) =
    duration start end
...
removeSelectedPeriods (Timesheet model) =
    let
        updatedPeriods =
            List.filter (Tuple.first >> not) model.finishedPeriods
    in
    Timesheet
        { model
            | finishedPeriods = updatedPeriods
    ...
    D.string
        |> D.andThen
            (\tab ->
                case tab of
                    "periods" ->
                        D.succeed PeriodsTab

                    "jira" ->
                        D.succeed JiraTab

                    _ ->
                        D.fail ("Invalid tab: " ++ tab)
            )
